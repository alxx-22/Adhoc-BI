...
// ---- Manager coverage & gaps: grouped live from PowerBIIntegration.Data by [Manager entitlement]
//      (same principle as colRepCompletion), so a report filter on the manager narrows it. ----
ClearCollect(
    colMgrDetail,
    ForAll(
        GroupBy(PowerBIIntegration.Data, 'Manager entitlement', 'MgrRows'),
        With(
            {
                trk: Filter(MgrRows, 'HPE Opportunity Id' in colAAOppIds.'Opp ID'),
                unt: Filter(MgrRows, !('HPE Opportunity Id' in colAAOppIds.'Opp ID'))
            },
            {
                MgrEmail:    'Manager entitlement',
                MgrName:     'Manager entitlement',
